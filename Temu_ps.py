...
#Use guideline for Temu_ps (Temu_PortScanner)
...
#Multi-port scan 
    ...
    # 1-500 --> port range between 1 through 500
    # 10 --> The number of threads thats going to be used. Recommended is aroung  100-200, don't overdo it.
    
#Single-port scan
    #Example use: python3 Temu_ps 192.168.1.4 80 10
    # 192.168.1.4 --> ip addr 
    # 80 --> Specific port that you want to lightly enumerate
...
#the number of threads that will be used during enumertaion process
num_of_threads = int(sys.argv[3])
HOST = str(sys.argv[1])

if '-' in sys.argv[2]:

    #List for ports and the same host ip address in order to stisfy map requirements
    port_list = []
    Host_list = []
    PORT = str(sys.argv[2])
